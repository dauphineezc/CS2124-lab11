// Zoe Dauphinee, rec 11, linked lists basics
...
void splice(Node* toSplice, Node* prior) {
    Node* prev_next = prior->next;
    prior->next = toSplice;
    Node* curr_node = toSplice->next;
    while (curr_node->next) {
        curr_node = curr_node->next;
    }
    curr_node->next = prev_next;
}
...
const Node* isSublist(const Node* pattern, const Node* target) {
    ...
    while (target) {
        ...
        while (curr_target && curr_pattern && curr_target->data == curr_pattern->data) {
            curr_target = curr_target->next;
            curr_pattern = curr_pattern->next;
        }
        if (!curr_pattern) return target;
...
const Node* sharedListBruteForce(const Node* listA, const Node* listB) {
    while (listA->next) {
        const Node* tempB = listB;
        while (tempB->next) {
            if (listA == tempB) { return listA; }
            else {
                tempB = tempB->next;
            }
        }
